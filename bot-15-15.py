...
def read_content(link_file):
    url_file = urllib.request.urlopen(link_file)
...
def vipuskniki(update, context):
    '''
    #для чтения из локальных файлов
    file = 'data/'  + 'vipuskniki.txt'
    content = read_content_from_file(file)
    '''
    #для чтения из удаленно расположенных файлов
    link_file = link + 'vipuskniki.txt'
    content = read_content_from_url(link_file)
